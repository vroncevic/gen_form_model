...
class SubProcessor:
    ...
    def run(self, *, params: Mapping[str, object]) -> Mapping[str, object]:
        '''
            ...
            :exceptions: None.
        '''
        model_name: str = 'myform'
        try:
            current_dir: str = dirname(realpath(__file__))
            output_dir: str = str(params.get('output') or './')
            model_name = str(params.get('name') or 'myform')
            model_type: str = str(params.get('type') or 'django')
            ...
            success = self._generator.generate(
                data=GeneratorData(
                    ...
                    template_key=model_type,
                    scheme=scheme,
                    template_values={
                        'project_name': model_name,
                        'name': model_name,
                        'PRO': model_name,
                        'MODC': model_name.capitalize(),
            ...
            return {
                ...
                'stdout': f'model {model_name} successfully generated.' if success else '',
                'stderr': f'failed to generate {model_name} model.' if not success else ''
            }
            ...
            return {
                'returncode': 1,
                'stdout': '',
                'stderr': f'failed to generate {model_name} model {exc}'
            }
